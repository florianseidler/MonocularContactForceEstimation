...
from keypoint_transformer.utils.preprocessing import *
from keypoint_transformer.utils.transforms import cam2pixel, pixel2cam, transformManoParamsToCam, convert_pose_to_opencv
import json
import smplx
import open3d as o3d
from tqdm import tqdm
from keypoint_transformer.utils.misc import get_root_rel_from_parent_rel_depths
ih26m_joint_regressor = np.load(cfg.joint_regr_np_path)
from keypoint_transformer.utils.misc import my_print
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode, annot_subset, capture=None, camera=None, seq_name_test=None, annotation_available=0, camera_data=False, object_name=''):
        ...
        for fname in tqdm(self.filelist[::1]):
            ...
            objTranslation = anno['objTrans']
            objRotation = anno['objRot']
            handJoints3D = anno['handJoints3D']
            
            if annotation_available:
                realhandtrans = anno['handTrans']
            else:
                realhandtrans = [0, 0, 0]
            ...
            if self.mode == False:  # 'evaluation':   # CHANGED!!'evaluation':
            ...
            data = {'img_path': img_path, 'seq_name': seq_name, 'cam_param': cam_param,
                    ...
                    'objName' : objName, 'objLabel' : objLabel, 'objTranslation' : objTranslation, 'objRotation' : objRotation, 
                    'handJoints3D' : handJoints3D, 'realhandtrans' : realhandtrans}
