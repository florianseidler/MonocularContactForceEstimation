...
from keypoint_transformer.utils.transforms import transformManoParamsToCam
import tqdm
...
def rotate_mano(mano_pose, mano_trans, mano_shape, joint_cam, rot, obj_rot, obj_trans):
    ...
    mano_pose_right, mano_trans_right, _ = transformManoParamsToCam(mano_pose[:48], mano_trans[:3], mano_shape[:10], new_rotmat_rod, np.zeros((3,)), 'right')
    mano_pose_left, mano_trans_left, _ = transformManoParamsToCam(mano_pose[48:], mano_trans[3:], mano_shape[10:], new_rotmat_rod, np.zeros((3,)),'left')
...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type, mano_pose, mano_trans, mano_shape, joint_cam,
    ...
    if mode == 'train':
        ...
        if mano_pose is not None:
            mano_pose, mano_trans, joint_cam, obj_rot, obj_trans = rotate_mano(mano_pose, mano_trans, mano_shape, joint_cam, rot, obj_rot, obj_trans)
    else:
    ...
    img, trans, inv_trans, inv_trans_no_rot, seg_patch = generate_patch_image(img, bbox, do_flip, scale, rot, cfg.input_img_shape, obj_seg)
...
def transform_input_to_output_space(joint_coord, joint_valid, rel_root_depth, root_valid, root_joint_idx, joint_type, skeleton):
    ...
    if cfg.dep_rel_to == 'root':
        ...
        joint_coord[joint_type['right'][:-1], 2] = joint_coord[joint_type['right'][:-1],2] - joint_coord[pid[joint_type['right'][:-1]],2]
        joint_coord[joint_type['left'][:-1], 2] = joint_coord[joint_type['left'][:-1], 2] - joint_coord[pid[joint_type['left'][:-1]], 2]
...
def generate_patch_image(cvimg, bbox, do_flip, scale, rot, out_shape, obj_seg):
    ...
    trans, trans_no_rot = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot)
    img_patch = cv2.warpAffine(img, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)
    if obj_seg is not None:
        obj_seg_patch = cv2.warpAffine(obj_seg.copy().astype(np.float32), trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)
    else:
        obj_seg_patch = None
    img_patch = img_patch.astype(np.float32)
    inv_trans, inv_trans_no_rot = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot, inv=True)
...
class PeakDetector():
    def __init__(self, nearest_neighbor_th=None):
        ...
        filename = 'output/dists_dict_%d.pickle' % (cfg.output_hm_shape[0])
        if not os.path.exists(filename):
            self.dists_dict = {}
            for i in tqdm.tqdm(range(self.pixel_inds.shape[0])):
                dists = np.linalg.norm(np.expand_dims(self.pixel_inds[i:i+1],1) - np.expand_dims(self.pixel_inds,0),axis=2) # 1 x N
    ...
    def detect_peaks_nms(self, image, num_peaks=200, nearest_neighbor_th=None, intensity_th=None):
        ...
        while np.sum(img > intensity_th) > 0:
            ...
            nearest_pixel_inds = self.pixel_inds[self.dists_dict[np.ravel_multi_index(ind, (image.shape[0],image.shape[1]))]]
